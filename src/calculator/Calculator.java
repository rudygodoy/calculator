...
public class Calculator {
    ...
    public static double process_postfix(String postfix, HashMap< String, Integer> map) {
        ...
        for (int i = 0; i < tokens.length; i++) // If token is a number or variable
        {
            if (opers.indexOf(tokens[i].charAt(0)) == - 1) {
                double term = 0.;
                try {
    ...
    public static void main(String[] args) {
        ...
        String opos = convert_to_postfix(op);
        System.out.println("Infix:   " + infix + " : " + op);
        System.out.println("Postfix: " + postfix + " : " + opos);
        HashMap< String, Integer> map = new HashMap< String, Integer>();
        HashMap< String, Integer> map2 = new HashMap< String, Integer>();
        for (int i = 0; i <= 100; i += 10) {
            map.put("C", i);
            System.out.println("C is " + i + ",  F is " + process_postfix(postfix, map));
            System.out.println("C is " + i + ",  F is " + process_postfix(opos, map));
        }
    }
}
